...
const setAnimations = (gltf: GLTF) => {
  ...
  const findFirstClip = (names: readonly string[]) =>
    names.map((n) => THREE.AnimationClip.findByName(animations, n)).find(Boolean);

  if (animations.length) {
    const introClip = findFirstClip(characterConfig.introAnimationNames);
    if (introClip) {
    ...
    characterConfig.loopedClips.forEach((name) => {
    ...
    typingAction = createBoneAction(
      gltf,
      mixer,
      characterConfig.typingClipName,
      typingBoneNames
    );
    if (typingAction) {
  ...
  function startIntro() {
    ...
    setTimeout(() => {
      const blink = findFirstClip(characterConfig.blinkAnimationNames);
      if (!blink) return;
  ...
  function hover(gltf: GLTF, hoverDiv: HTMLDivElement) {
    let eyeBrowUpAction = createBoneAction(
      gltf,
      mixer,
      characterConfig.browUpClipName,
      eyebrowBoneNames
    );
